...
function gotourl(vasid){
    window.location.href = config.url+vasid;
}
        ...
        if (config.type == 'bar') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                template += '<div class="sc-item" style="width:' + itemWidth[i] + '%;' + barColor + 'z-index:' + (config.item.value.length - i) + '"  thisvalue="'+vasid+'">';
        ...
        if (config.type == 'column') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                barColor = getBackgroundColor();
                vasid = i+1;
                template += '<div class="sc-item" style="left:' + ((itemWidth + barMargin) * i + barMargin) + '%;width:' + (itemWidth) + '%;height:' + itemHeight[i] + '%;' + barColor + '" thisvalue="'+vasid+'">';
                template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
                template += '<div class="sc-tooltip">';
                template += '<span class="sc-tooltip-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
        ...
        if (config.type == 'step') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                barColor = getBackgroundColor();
vasid = i+1;
                template += '<div class="sc-item' + stepClass + '" style="left:' + ((itemWidth + barMargin) * i) + '%;width:' + (itemWidth) + '%;top:' + (100 - itemPercentage[i]) + '%;' + barColor + '" thisvalue="'+vasid+'">';
                template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
                template += '<div class="sc-tooltip">';
                template += '<span class="sc-tooltip-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
        ...
        if (config.type == 'progress') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                vasid = i+1;
                (i - 1 >= 0) ? leftPosition[i] = leftPosition[i - 1] + itemWidth[i - 1] : leftPosition[0] = 0;
                template += '<div class="sc-item" style="left:' + leftPosition[i] + '%;width:' + itemWidth[i] + '%;' + barColor + 'z-index:' + (config.item.value.length - i) + '" thisvalue="'+vasid+'">';
                template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
                template += '<div class="sc-tooltip">';
                template += '<span class="sc-tooltip-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
        ...
        if (config.type == 'waterfall') {
            ...
            for (i = 0; i < config.item.value.length; i++) {
                ...
                barColor = getBackgroundColor();
                vasid = i+1;
                template += '<div class="sc-item" style="left:' + leftPosition[i] + '%;width:' + itemWidth[i] + '%;' + barColor + '" thisvalue="'+vasid+'">';
                template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
                template += '<div class="sc-tooltip">';
                template += '<span class="sc-tooltip-value">' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
        ...
        $(".sc-item").click(function(){
            thisvalue = $(this).attr('thisvalue');
            console.log("VASID",thisvalue);
            gotourl(thisvalue);
        })
