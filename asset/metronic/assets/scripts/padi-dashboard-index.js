    ...
    return {
        ...
        initCharts: function () {
            ...
            function showTooltip(title, x, y, contents) {
                $('<div id="tooltip" class="chart-tooltip"><div class="date">' + title + '<\/div><div class="label label-success">Visit: ' + y  + '<\/div><div class="label label-important"><\/div><\/div>').css({
                    position: 'absolute',
                    display: 'none',
                    top: y - 100,
                    width: 75,
            ...
            .done(function(res){
                ...
                data = res
                loadVisitStatistics(data)
            })
            .fail(function(err){
                console.log("Error visitjson",err)
            });

            $.ajax({
                url:'/dashboards/offerjson',
                type:'post',
                dataType:'json'
            })
            .done(function(res){
        ...
        loadVisitStatistics = function(data){
            if ($('#site_statistics').size() != 0) {
                ...
                $.each(data,function(x,y){
                    chart_data.push({data:y.visit,label:y.sales});
                })
                var plot_statistics = $.plot($("#site_statistics"), chart_data, {
                    series: {
                        lines: {
                            show: true,
                            lineWidth: 2,
                ...
                $("#site_statistics").bind("plothover", function (event, pos, item) {
                    ...
                    if (item) {
                        if (previousPoint != item.dataIndex) {
                            ...
                            showTooltip(' '+getCurMonth(x)+' '+getCurYear() +' : '+ parseInt(y), item.pageX, item.pageY, '');
                        }
                    } else {
                        $("#tooltip").remove();
                        previousPoint = null;
                ...
                loadOfferStatistics = function(data){
                    if ($('#offer_statistics').size() != 0) {
        
                        $('#offer_statistics_loading').hide();
                        $('#offer_statistics_content').show();
                        chart_data = [];
                        $.each(data,function(x,y){
                            chart_data.push({data:y.offer,label:y.sales});
                        })
                        var plot_statistics = $.plot($("#offer_statistics"), chart_data, {
                            series: {
                                lines: {
                                    show: true,
                                    lineWidth: 2,
                        ...
                        $("#offer_statistics").bind("plothover", function (event, pos, item) {
                            //$("#x").text(pos.x.toFixed(2));
                            //$("#y").text(pos.y.toFixed(2));
                            if (item) {
                                if (previousPoint != item.dataIndex) {
